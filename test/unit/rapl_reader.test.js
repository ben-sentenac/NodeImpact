...
test('PRIME:first tick should return the right prime value', async () => {
...
test('delta without wra correction end delta_j and power_w correct', async () => {
...
test('wrap correction reading max_energy_range_uj', async () => {
...
test('unreadable package: ignored if at least one package is readable', async() => {
    //
     const [p0,p1] = await Promise.all(
        [
            createRaplPackages(tmp, 'intel-rapl:0', { name: 'package-0', energy: 1_000_000n }),
...
test('clamp delta_ts: dt trop petit/grand', async () => {
